...
class LocallyConnected2d(nn.Module):
    def __init__(self, batch_size, in_channels, out_channels, bias_size, bias=False):
        ...
        for i in range(batch_size):
            ...
            #self.normlayers.append(nn.Sequential(nn.BatchNorm1d(in_channels//self.batch_size).cuda(), nn.LeakyReLU(negative_slope=0.2).cuda()))
        '''
        if bias:
    ...
    def forward(self, x):
        ...
        for i in range(self.batch_size):
            row = self.convlayers[i](x[height*i:height*(i+1),:].unsqueeze(0).unsqueeze(0))[0][0]
